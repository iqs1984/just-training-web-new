...
export class AssignPlayers extends React.Component {
    ...
    componentDidMount() {
        ...
        $crud.retrieve("groups").then(data => {
            const {groups} = data;
            this.setState({
                groups: groups
            });
                        ...
                        <table className="table table-bordered table-hover">
                            ...
                            <tbody>
                            {
                                group && players.filter(player => (searchKeyword ? player.name.toLowerCase().includes(searchKeyword.toLowerCase()) : player.name === player.name) && !_.find(group.players, p => p.id === player.id)).map((player, i) =>
                                    // !_.find(group.players, p => p.id === player.id)).map((player, i)
                ...
                <div className="col-lg-2 text-center" style={{
                    ...
                    <div className="mb-3">
                        <button onClick={() => this.assign(selectedPlayers)}
                                className="btn btn-md btn-primary assign-players"><i className="fa fa-angle-right"></i>
                        </button>
                    </div>
                    <div>
